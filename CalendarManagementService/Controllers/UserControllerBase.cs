...
namespace CalendarManagementService.Controllers
{
    ...
    public abstract class UserControllerBase : ControllerBase
    {
        ...
        public abstract Task<IActionResult> AddUser([FromRoute][Required][RegularExpression("^(?<major>[0-9]+).(?<major>[0-9]+)$")] string version, [Required, EmailAddress] string userEmail, [Required] string userName, [Required] string password);
        ...
        [SwaggerResponse(statusCode: 400, type: typeof(ActionResult), description: "Bab Request")]
        [SwaggerResponse(statusCode: 401, type: typeof(ActionResult), description: "Unauthorized")]
        public abstract Task<IActionResult> Login([FromRoute][Required][RegularExpression("^(?<major>[0-9]+).(?<major>[0-9]+)$")] string version, [Required, EmailAddress] string userEmail, [Required] string password);
